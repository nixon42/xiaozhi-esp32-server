...
hass_play_music_function_desc = {
    ...
    "function": {
        ...
        "description": "Used when the user wants to listen to music or an audiobook. Plays the corresponding audio on a room's media player (media_player).",
        "parameters": {
            "type": "object",
            "properties": {
                "media_content_id": {
                    "type": "string",
                    "description": "Can be the album name, song title, or artist of the music or audiobook. If not specified, use 'random'.",
                },
                "entity_id": {
                    "type": "string",
                    "description": "The entity_id of the speaker device in Home Assistant, starts with media_player.",
                },
            },
...
def hass_play_music(conn: "ConnectionHandler", entity_id="", media_content_id="random"):
    try:
        ...
        return ActionResponse(
            action=Action.RESPONSE, result="Music playback handled", response=ha_response
        )
