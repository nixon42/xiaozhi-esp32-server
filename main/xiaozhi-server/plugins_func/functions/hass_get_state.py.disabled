...
hass_get_state_function_desc = {
    ...
    "function": {
        "name": "hass_get_state",
        "description": "Gets the state of a device in Home Assistant, including querying light brightness, color, color temperature, media player volume, and pause/resume operations.",
        "parameters": {
            "type": "object",
            "properties": {
                "entity_id": {
                    "type": "string",
                    "description": "The entity_id of the device to query in Home Assistant.",
                }
            },
            "required": ["entity_id"],
        },
...
def hass_get_state(conn: "ConnectionHandler", entity_id=""):
    try:
        ...
        logger.bind(tag=TAG).error("Home Assistant state query timed out")
